...
// Returns a float as the length between the two input points
...
// Using three (x, y) points this classifies the triangle
...
void triangle() {
  while (true) { // while(true) loop will run as long as there are points being
                 // entered
    int x1, y1, x2, y2, x3, y3; // represent three x, y points
    bool triangle =
        true; // if true then the points are classified as a type of triangle
    ...
    // if points lie on the same line it is not a triangle (Degenerate. Not a
    // triangle.)
    if ((x1 == x2 && x2 == x3) ||
        (y1 == y2 && y2 == y3)) { // need to fix and finish
      triangle = false;
