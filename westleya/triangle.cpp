...
std::vector<int> stringToVec(std::string s){
    ...
    for(int i = 0; i < s.length(); i++){
        ...
        while (isdigit(s[i])){ // http://www.cplusplus.com/reference/cctype/isdigit/
            next += s[i];
            i++;
        }
        
        if (next.length() > 0) {
            int next_num = std::stoi (next); // http://www.cplusplus.com/reference/string/stoi/
...
bool isDegenerate(std::vector<int> tri_pts) {
    ...
    if ((std::abs(tri_pts[0] - tri_pts[2]) == 0 && std::abs(tri_pts[4] - tri_pts[2]) == 0) ||
        (std::abs(tri_pts[0] - tri_pts[2]) == 0 && std::abs(tri_pts[1] - tri_pts[3]) == 0) ||
        (std::abs(tri_pts[3] - tri_pts[5]) == 0 && std::abs(tri_pts[4] - tri_pts[2]) == 0) ||
        (std::abs(tri_pts[0] - tri_pts[4]) == 0 && std::abs(tri_pts[5] - tri_pts[1]) == 0) ||
        (fabs((tri_pts[1] - tri_pts[3])/(tri_pts[0] - tri_pts[2] - 0.0) -
              (tri_pts[3] - tri_pts[5])/(tri_pts[2] - tri_pts[4] - 0.0)) < 0.00001)) {
        return true;
    }
...
void classifyTriangle(std::vector<int> tri_pts) {
    ...
    else if (fabs(side_a - side_b) < tolerance || fabs(side_b - side_c) < tolerance ||
